...
import { processAssignmentMarksImage } from '../../services/geminiService.js';
...
const generatePdf = (data) => {
    ...
    doc.setFontSize(12);
    doc.text('SUMMARY OF THEORY ATTENDANCE Assignments', 20, 45);
    ...
    const head = [['Roll No.', 'NAME OF STUDENT', 'Assignment 1', 'T', 'Assignment 2', 'T', 'Avg. Marks']];
    const body = extractedStudents.map(s => {
        const assignment1 = s.assignment1 ?? '';
        const assignment2 = s.assignment2 ?? '';
        const avgMarks = s.avgMarks ?? '';
        return [s.rollNo, s.studentName, assignment1, assignment1, assignment2, assignment2, avgMarks];
    });
    ...
    doc.autoTable({
        ...
        styles: { fontSize: 9, cellPadding: 4, halign: 'center', lineWidth: 0.5, lineColor: [0,0,0] },
...
export function renderUploadAssignmentMarks(rootElement, props) {
    const { students, onSaveAssignmentMarks } = props;
    ...
    const handleProcessDocument = async (files) => {
        ...
        try {
            ...
            const result = await processAssignmentMarksImage(images);
    ...
    const handleUploadPdf = () => {
        ...
        data.students.forEach(extractedStudent => {
            ...
            if (studentId) {
                if (extractedStudent.assignment1 !== undefined) {
                    updates.push({ studentId, markType: 'assignment1', value: extractedStudent.assignment1 });
                }
                if (extractedStudent.assignment2 !== undefined) {
                    updates.push({ studentId, markType: 'assignment2', value: extractedStudent.assignment2 });
                }
        ...
        if (updates.length > 0) {
            ...
            onSaveAssignmentMarks(courseCode, updates, pdfDataUrl);
    ...
    const renderExtractedData = () => {
        ...
        const studentRows = extractedStudents.map(s => `
            <tr class="border-b hover:bg-gray-50">
                ...
                <td class="p-2 text-center">${s.assignment1 ?? '-'}</td>
                <td class="p-2 text-center">${s.assignment2 ?? '-'}</td>
                    ...
                    <table class="w-full text-left table-auto text-sm">
                        <thead class="bg-gray-100 sticky top-0">
                            <tr>
                                ...
                                <th class="p-2 font-semibold text-center">Assignment 1</th>
                                <th class="p-2 font-semibold text-center">Assignment 2</th>
    ...
    const render = () => {
        ...
        const content = `
            <div>
                <h2 class="text-3xl font-bold text-gray-800">Upload Assignment Marksheet</h2>
                <hr class="my-6"/>
                <div class="grid grid-cols-1 lg:grid-cols-5 gap-12">
                    <div class="lg:col-span-2">
                        ${Card({
                            ...
                            children: `
                                <form id="upload-assignment-form" class="space-y-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Marksheet File(s) (Image)</label>
                                        <div class="flex items-center">
                                            <label for="assignment-file" class="cursor-pointer bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50">Choose Files</label>
                                            <input type="file" id="assignment-file" accept="image/png, image/jpeg, image/webp" class="sr-only" multiple/>
                                            <span id="assignment-file-name" class="ml-3 text-sm text-gray-500 truncate">${state.selectedFiles.length > 1 ? `${state.selectedFiles.length} files selected` : (state.selectedFiles[0]?.name || 'No files chosen')}</span>
                                        </div>
                                    </div>
    ...
    const attachEventListeners = () => {
        document.getElementById('upload-assignment-form')?.addEventListener('submit', e => {
            e.preventDefault();
        ...
        document.getElementById('export-pdf-btn')?.addEventListener('click', () => {
            ...
            const doc = generatePdf(data);
            doc.save(`${data.subjectDetails.subjectCode}-Assignment-Marks.pdf`);
        });
        ...
        document.getElementById('export-excel-btn')?.addEventListener('click', () => {
            ...
            const excelData = students.map(s => ({
                ...
                'Assignment 1': s.assignment1 ?? null,
                'Assignment 2': s.assignment2 ?? null,
                'Avg. Marks': s.avgMarks ?? null,
            }));
            const ws = XLSX.utils.json_to_sheet(excelData);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Assignment Marks");
            XLSX.writeFile(wb, `${subjectDetails.subjectCode}-Assignment-Marks.xlsx`);
        });
    };
